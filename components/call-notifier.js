...
class CallNotifier extends HTMLElement {
    async connectedCallback() {
        // Prevent interference if we are already on the call page
    ...
    render() {
        ...
        <style>
            ...
            .n-acc { background: #00d2ff; color: #000; }
        </style>
        <div id="notif-banner">
            <div class="c-prof">
            ...
            <div class="n-btns">
                ...
                <button class="n-btn n-acc" id="n-view">View</button>
            </div>
    ...
    initSwipe() {
        ...
        banner.addEventListener('touchmove', (e) => {
            ...
            if (Math.abs(currentX) > 0) {
                // Apply a slight rotation and move
    ...
    listen() {
        ...
        this.signalRef.on('value', snap => {
            ...
            if (data) {
                ...
                // DECLINE: Update call state and remove signal
                ...
                // VIEW: Disable listener and navigate
                this.querySelector('#n-view').onclick = (e) => {
                    e.stopPropagation();
                    this.signalRef.off(); // STOP LISTENING SO CALLS.HTML CAN TAKE OVER
                    this.hide();
                    window.location.href = `calls.html?callId=${data.callId}`;
                };

                // AUTO-HIDE if caller cancels
                firebase.database().ref(`calls/${data.callId}/status`).on('value', s => {
                    if (['cancelled', 'ended'].includes(s.val())) this.hide();
                });
            } else {
                this.hide();
            }
